...
const int BUBBLE_DEFAULT_AY = 100;  // acceleration due to gravity
...
public:
    ...
    void nextStep(double t)
    {
        ...
        vy = vy + ay *t;
        double new_y = circle.getY() + (vy)*t ;   //new_y will have influence of gravity
        ...
        if ((vy  < 0 && new_y < (0 + circle.getRadius())) // bounce along Y direction at top border
            ||
            (vy + ay*t > 0 && new_y > (450 - circle.getRadius()))) // bounce along Y direction at bottom line which is at PLAY_Y_HEIGHT equal to 450
        {
